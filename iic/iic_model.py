...
class IICModel(pl.LightningModule):
    def __init__(self, input_channels, nr_classes, input_size):
        ...
        self.val_sample = val_set[0:]["img1"].cuda()
        self.step = 0
    ...
    def training_step(self, train_batch, batch_idx):
        loss, acc = self.train_evaluation(train_batch)
    ...
    def validation_step(self, train_batch, batch_idx):
        ...
        sample_clf = model(self.val_sample).detach().cpu()
        sample_clf = recombine_image(sample_clf, RECOMBINATION_SIZE, int(SUB_IMAGE_SIZE * CROP_FACTOR))
        self.step += 1
        for ch in range(self.nr_classes):
            self.logger.experiment.add_image(f'generated_map_{ch}', sample_clf[ch, :, :], self.step, dataformats="HW")
